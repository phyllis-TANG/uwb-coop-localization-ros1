...
4. 现场已在 Python 2.7.17/pyserial 3.4 下通过 6 项解析器测试，并使用
   ...
   `/uwb/ranges` 发布 `tag_id: node_1` 和空 `ranges` 数组。下一关只需给 P-A
   上电，一次验证 `node_0` 的实时距离与发布频率。现场已看到 1.530、1.574、
   ...
   距离。P-A/P-B 台架验证至此完成，下一步转入小车断电安装检查。
5. 完成现场回放和静止设备测试后，再验证 ROS 发布距离。不要在驱动中
...
## 10. EXP001–004 离线分析阶段（2026-09-22）

- 用户报告 EXP001–004 均已完成，覆盖静止 LOS、静止人体 NLOS、手推 LOS 和手推
  人体 NLOS；正式记录位于 `docs/field_tests/`。
- 已增加 Python 2.7 兼容的只读 rosbag 分析器。它使用 bag 接收时间，保留空帧和
  原始尖峰，不使用 NaN `quality`，并输出 CSV、统计摘要、SHA256 与 PNG。
- 仓库不保存原始 bag，因此云端不能声称已经重新计算 SHA256 或复现最终图表。
  现场必须使用基线 JSON 运行验收；差异解决前不得改写已有实验结论。
- 用户于 2026-09-22 在保存原始文件的虚拟机上计算并回报四份 bag 的 SHA256；
  哈希已写入各实验报告。云端未持有这些 bag，仍需现场运行分析验收。
- 现场已成功复现四组任务要求的帧数、空帧数、均值、中位数和标准差。旧交接中的
  跳变数值无法由“跳过空帧”或“连续消息均有效”两种定义完整复现，说明旧流程含
  未记录规则；报告保留旧值并另列当前明确规则下的可复现值，机器基线不再把旧
  跳变值作为通过条件。
- 2026-09-22 最终现场分析使用修订后的基础验收清单返回状态 0，并完整生成四份
  CSV/摘要/PNG、汇总 CSV/JSON 和对比 PNG；输出目录约 2.9 MiB。数值验收完成，
  合并前还需人工目视检查最终图像。
- 单车 UWB 初期测距实验完成。下一阶段依次为双车静止测距、两车 UWB/odom/IMU/tf
  同步录制，以及跨计算机时间同步处理。
