...
- EXP002–004 已完成，覆盖静止人体 NLOS、手推 LOS 和手推人体 NLOS；正式记录
  与四实验对比位于 `docs/field_tests/`。
- Python 2.7 只读分析器使用 bag 接收时间，保留空帧与原始尖峰，不使用 NaN
  `quality`，并输出 CSV、统计摘要、SHA256 与 PNG。
- 四份原始 bag 的 SHA256 已在现场虚拟机计算并写入报告；原始 bag 不提交 Git。
- 最终现场基础验收返回状态 0，完整生成四份 CSV/摘要/PNG、汇总 CSV/JSON 和
  对比 PNG；输出目录约 2.9 MiB。
- 旧交接跳变值因计算规则未完整记录而无法严格复现。报告保留旧值，并另列“连续
  两条 ROS 消息均有效”定义下的可复现值，不将旧跳变值作为基础验收条件。
- 单车初期测距完成；下一阶段为双车静止测距、两车 UWB/odom/IMU/tf 同步录制和
  跨计算机时间同步处理。
