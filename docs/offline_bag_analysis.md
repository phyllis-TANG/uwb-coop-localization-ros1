...
`--expectations` 会核对验收要求明确列出的四组帧数、空帧数、均值、中位数和
标准差。如果任何一项
...
中存在 0.175 m 的连续消息跳变，而旧记录最大值为 0.172 m。旧计算显然还使用了
未记录的时间间隔筛选、舍入或其他规则。为避免迎合脚本而改写历史，旧值在报告中
保留并标为未复现；机器验收清单只包含任务明确要求必须复现的基础统计。新的可复现
跳变结果统一采用本文记录的“连续 ROS 消息均有效”定义。
