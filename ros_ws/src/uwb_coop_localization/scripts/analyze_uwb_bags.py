...
TOPIC = "/uwb/ranges"
EXPERIMENT_TITLES = {
    "EXP001": "EXP001: Static LOS",
    "EXP002": "EXP002: Static human obstruction",
    "EXP003": "EXP003: Manual push LOS",
    "EXP004": "EXP004: Manual push human obstruction",
}
...
def plot_comparison(path, plot_inputs):
    ...
    for index, (title, records) in enumerate(plot_inputs):
        ...
        axis.set_title(EXPERIMENT_TITLES.get(title, title))
    ...
    # Preserve direct LOS/NLOS visual comparison by sharing the y scale within
    # each row while allowing static and dynamic rows to use different scales.
    for row in range(rows):
        row_axes = [axis for axis in axes[row] if axis in visible_axes]
        if not row_axes:
            continue
        lower = min(axis.get_ylim()[0] for axis in row_axes)
        upper = max(axis.get_ylim()[1] for axis in row_axes)
    ...
    figure.text(
        0.5, 0.015,
        "Same y-scale within each row; separate recordings. No smoothing. "
        "Red ticks are missing/invalid target ranges.",
        ha="center")
